...
public final class CardEmulation {
    ...
    /**
     * Setting NFC controller routing table, which includes Protocol Route and
     * Technology Route, while this Activity is in the foreground.
     *
     * The parameter set to null can be used to keep current values for that
     * entry. <p> Example usage in an Activity that requires to set proto route
     * to "ESE" and keep tech route: <pre> protected void onResume() {
     ...
     * @param activity The Activity that requests NFC controller routing table
     *     to be changed.
     * @param protocol ISO-DEP route destination, which can be "DH" or "UICC" or
     *     "ESE".
     * @param technology Tech-A, Tech-B route destination, which can be "DH" or
     *     "UICC" or "ESE".
     ...
     * @hide
     */
    public boolean overrideRoutingTable(Activity activity, String protocol,
                                        String technology) {
        if (activity == null) {
            throw new NullPointerException(
                "activity or service or category is null");
        }
        if (!activity.isResumed()) {
            throw new IllegalArgumentException("Activity must be resumed.");
        }
        try {
            return sService.overrideRoutingTable(UserHandle.myUserId(),
                                                 protocol, technology);
        } catch (RemoteException e) {
            // Try one more time
            recoverService();
     ...
     * which was changed by {@link #overrideRoutingTable(Activity, String,
     * String)}
     *
     * @param activity The Activity that requested NFC controller routing table
     *     to be changed.
     * @return true if operation is successful and false otherwise
     *
     * @hide
